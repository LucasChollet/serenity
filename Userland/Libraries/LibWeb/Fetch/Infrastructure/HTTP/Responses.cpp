...
u64 Response::current_age() const
{
    ...
    if (header_list()->contains("Age"sv.bytes())) {
        auto const age_view = header_list()->get("Age"sv.bytes()).value();
        if (auto converted_age = MUST(String::from_utf8(age_view.span())).to_number<u64>(); converted_age.has_value())
...
{
    if (!header_list()->contains("Cache-Control"sv.bytes()))
        return 0;
    auto const elem = header_list()->get_decode_and_split("Cache-Control"sv.bytes()).value();
    ...
    for (auto const& directive : elem) {
        if (directive.starts_with_bytes("max-age"sv)) {
            ...
            auto const value = MUST(directive.substring_from_byte_offset(equal_offset));
